...
function [matrice_coefficients] = buildmatrix(size, diffusion_coefficient)
    ...
    for i = 1:size
       ...
       global sigma_a
       if i == 1
           line(i) = -(diffusion_coefficient(i) + 2*diffusion_coefficient(i+1) + diffusion_coefficient(i+2) + sigma_a(i+1));
           line(i+1) = diffusion_coefficient(i+1) + diffusion_coefficient(i+2);
       elseif i == size
           line(i-1) = diffusion_coefficient(i-1) + diffusion_coefficient(i);
           line(i) = -(diffusion_coefficient(i-1) + 2*diffusion_coefficient(i) + diffusion_coefficient(i+1)+ sigma_a(i+1));
       else
